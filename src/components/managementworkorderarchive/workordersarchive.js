...
import "./workordersarchive.css";
import moment from "moment";
import axios from "axios";

class WorkOrdersArchive extends React.Component {
  constructor(props) {
    super(props);
  ...
  updateArchive () {
     axios.get(`/search_work_orders_archive/${this.state.unitNumber}`).then(response => {
      this.props.updateWorkOrdersArchive(response.data)
    });
  ...
  render() {
    ...
    return (
      <div className="archiveworkordercontainer">
        <div className="searchbar">
          <p>Unit Number Search:</p>{" "}
          <select
            className="createnewselect"
            value={this.unitNumber}
          ...
          <button className = "searchbutton" onClick={this.updateArchive}>Search</button>
          <button className = "searchbutton" onClick={this.resetSearch}>Reset</button>
        </div>

        {this.props.workOrdersArchive.map((workOrder, index) => (
          <div className="archiveindividualWorkOrder" key={index}>
            <div className="archivewodate">
              Submitted: {moment(workOrder.created_at).format("lll")}
            </div>
            <div className="archiveinformationcontainer">
              <div className="archiveunitandname">
                <p className="archiveunitNumber"> {workOrder.unit_number}</p>
                <p className="archivetenantName"> {workOrder.tenant_name}</p>
              </div>
              <div className="label">
                <h1>Problem:</h1>
                <h1>Notes:</h1>
              ...
              <div className="issuenotescontainer">
                <p className="archiveissue"> {workOrder.issue}</p>
                <p className="archivenotes"> {workOrder.notes}</p>
              </div>
            </div>
            <div className="archivewodate">
              Completed: {moment(workOrder.completed_at).format("lll")}
            </div>
          </div>
        ))}
      </div>
...
export default WorkOrdersArchive;
